...
AI-amnTV 不是一打开就把用户扔进后台表格：

1. **官网 / 发现首页**：解释产品、展示真实项目与镜头、承接创作灵感、Skills、导入和最近项目。
2. **项目创作台**：以可拖拽生产图为主界面，在同一上下文中切换工作流、故事板和制作 Agent。

首页负责让创作者理解“为什么值得开始”，项目画布负责让团队真正把一集做完。
...
- 10 个可见节点：8 个必需阶段，以及外部生成、质量评测 2 个可选工位
- 四类人工关卡：剧本、定妆、分镜与关键帧、最终成片
- 节点可拖动、缩放和自动整理，依赖关系直接显示在画布连线上
- 工作流状态由项目文件、镜头状态和任务账本实时计算，不用前端假进度
- 每个节点显示进度、阻塞原因、引用素材和下一步动作
- 故事板是同一项目的镜头墙视图，不再是割裂的独立页面
- 右侧 Agent 可以引用当前节点与资源，只先给执行计划，付费操作仍需确认
...
浏览器会打开 `http://127.0.0.1:4317`。根页面先展示官网 / 发现首页；点击“进入工作台”
即可查看 15 卡示例分集的可拖拽生产图、节点依赖、制作 Agent 和故事板镜头墙。
...
| 页面 | 用途 |
|---|---|
| 官网首页 | 产品介绍、创作灵感、Skills、最近项目与真实镜头作品区 |
| 导入 | 预检并导入完整项目、YAML / JSON 剧本、TXT / Markdown 小说或大纲 |
| 创作画布 | 拖拽生产节点、查看依赖、切换故事板并让 Agent 引用当前上下文 |
| 项目总览 | 查看项目规格、关卡、Provider 状态、成本与镜头进度 |
| 剧本 | 编辑结构化场景、台词与情绪契约 |
| 分镜 | 编辑逐卡景别、运镜、人物、台词覆盖和生成模式 |
| 外部引擎 | 使用受控 LibTV 会话创作、续写、回收和晋升结果 |
