...
侧栏的“导入”进入两阶段导入中心：先只读预检，再确认写入。当前支持：

1. **AI-amnTV 项目目录**：粘贴包含 `series.yaml` 的本机目录路径，连同分集、媒体、候选和历史
   状态完整复制；已经位于当前项目库中的目录会直接打开。
2. **结构化剧本**：选择 YAML 或 JSON。既可以直接使用一个 `Script` 对象，也可以使用包含
   `series`、`script`、`storyboard`、`characters`、`locations` 的对象。缺少但已被剧本引用的
   角色或场景会建立待完善档案。
3. **小说或大纲**：选择 TXT/Markdown，或直接粘贴正文。原文保存在
   `episodes/<EP-ID>/source.md`，导入后进入已预填的大纲创作页；只有用户确认后才会提交编剧
   Agent。

导入不会覆盖同名系列。完整项目通过隐藏暂存目录复制并校验后一次性更名，失败不会留下半成品；
项目导入拒绝符号链接，忽略所有隐藏文件与目录（包括 `.studio`、`.env` 和 `.DS_Store`），文本导入
上限为 1.5MB。
